...
<body>
	...
	<div class="page-container">
		<form action="${pageContext.request.contextPath }/user/getUserList" method="get">
		<div class="text-c">
			...
			<input type="text"  placeholder=" 用户名称/手机号/身份证" name="text" value="${searchVo.text}"
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="return deleteChoice('${pageContext.request.contextPath }/user/deleteUserByChoice')" class="btn btn-danger  radius"><i class="Hui-iconfont">&#xe6e2;</i>批量删除</a>
					<a href="javascript:;" onclick="alertShow('添加用户','${pageContext.request.contextPath }/dispatcher?view=/admin/user/user-add','1000','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 新建用户</a>
				...
				<tbody>
					<c:if test="${userList != null }">
						<c:forEach items="${userList }" var="user" varStatus="status">
      					<tr class="text-c">
						<td><input type="checkbox" value="${user.id}" name="choice"></td>
						<td>${status.index+1}</td>
						<td class="td-status"><span
							class="label label-success radius">${user.username}</span></td>
						<td class="td-status"><span
							class="label label-success radius">${user.mobile}</span></td>
						<td class="td-status">${user.roleName}</td>
						<td class="td-status"><span
							class="label label-success radius">${user.identity}</span></td>
						<td class="td-status"><span
							class="label label-success radius">${user.balance}</span></td>
						<td class="td-status"><span
							class="label label-success radius"><fmt:formatDate value="${user.createTime}" pattern="yyyy年MM月dd日HH点mm分ss秒" /></span></td>
						<td class="td-status"><span
							class="label label-success radius"><fmt:formatDate value="${user.updateTime}" pattern="yyyy年MM月dd日HH点mm分ss秒" /></span></td>
						<td class="f-14 td-manage">
							 <a style="text-decoration: none" class="ml-5"
							onClick="edit('修改信息','${pageContext.request.contextPath }/user/getUserById/${user.id}?view=admin/user/user-edit','10001')"
							href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
							<a style="text-decoration: none" class="ml-5"
							onClick="deleteOne(this,'${pageContext.request.contextPath }/user/delete/${user.id}')" href="javascript:;"
